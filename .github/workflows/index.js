...
bot.on('message', async message=> {
	...
	switch(args[0]){			
		...
		case 'k1llfeed':
			...
			if (args[1] === 'PAUSE') {
                if (!message.member.roles.cache.some(r => r.name === "Adm")) return message.channel.send('YOU DO NOT HAVE THE REQUIRED PERMISSIONS') .then (message => message.delete({ timeout: 5000, }));
                feedStart = false;
                return;
            }
            if (args[1] === 'RESUME') {
								...
								for ( val of data2){
									if (val[15]){
										...
										if(dt0 != tRef && dt0 > tRef) {
											...
											message.channel.send(embed).then (message => message.delete({ timeout: 180000000, }));
											tRef = dt0;
										}
